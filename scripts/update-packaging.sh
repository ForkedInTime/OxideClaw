...
# contrib/homebrew/rustyclaw.rb to a released tag, using the per-asset
# SHA-256 files the release workflow publishes.
#
#   scripts/update-packaging.sh v0.3.3
#
# Then: push contrib/homebrew/rustyclaw.rb to the tap repo and
...
lx=$(sum rustyclaw-linux-x64)
la=$(sum rustyclaw-linux-arm64)
mx=$(sum rustyclaw-macos-x64)
ma=$(sum rustyclaw-macos-arm64)
...
rb="${root}/contrib/homebrew/rustyclaw.rb"
